...
func GetConseils(w http.ResponseWriter, r *http.Request) {
	...
	query := `SELECT c.Id_Conseils, COALESCE(c.Titre, ''), c.Contenu, COALESCE(c.Categorie, ''), 
		COALESCE(c.Tags, ''), c.Date_d_ajout, u.Nom, u.Prenom, COALESCE(s.Poste, '')
		FROM Conseils c
...
func GetConseil(w http.ResponseWriter, r *http.Request) {
	...
	row := database.DB.QueryRow(`
		SELECT c.Id_Conseils, COALESCE(c.Titre, ''), c.Contenu, COALESCE(c.Categorie, ''),
			COALESCE(c.Tags, ''), c.Date_d_ajout, u.Nom, u.Prenom, COALESCE(s.Poste, '')
		FROM Conseils c
		JOIN Salaries s ON s.Id_Salaries = c.Id_Salaries
		JOIN Utilisateurs u ON u.Id_Utilisateurs = s.Id_Utilisateurs

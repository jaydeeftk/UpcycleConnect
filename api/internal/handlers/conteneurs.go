...
func GetAnnoncesEligiblesDepot(w http.ResponseWriter, r *http.Request) {
	liste, err := conteneurSvc.ListerAnnoncesEligiblesPourDepot(middleware.GetUserID(r))
	if err != nil {
		httpx.WriteError(w, err)
		return
	}
...
func CreateDemandeConteneur(w http.ResponseWriter, r *http.Request) {
	...
	id, err := conteneurSvc.CreerDemande(middleware.GetUserID(r), services.CreationDepotInput{
		EtatUsure: body.EtatUsure, IdConteneur: body.IdConteneur,
		DateDepot: body.DateDepot, PhotoUrl: body.PhotoUrl, IdAnnonce: body.IdAnnonce,
	})
	if err != nil {
		httpx.WriteError(w, err)
		return

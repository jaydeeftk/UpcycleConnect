...
Légende : ✅ couvert et prouvé · ⚠️ dette connue (voir §4).
...
- **Preuve** : commits `c819d61` (vertical), `2d38caa` (inscription/identité). CHECK statut en base. ✅
...
- **Preuve** : commits `980c51a` (1 box/conteneur), `3e0d542` (vertical), `e22cae5` (occupation `reserve_pro`). Harnais conteneur **54/0** (régression #27). ✅
...
- **Preuve** : commits `031e46c` (CHECK), `b14966b` (vertical), `3bf50a0` (paiement préalable). ✅
...
- **Preuve** : commits `561d6b3` (CHECK), `78ef8a9` (vertical + identité JWT). ⚠️ le front doit attacher l'`Authorization` sur les POST/PATCH forum.
...
- **Preuve** : commits `e22cae5`, `2ad7ae8`. Harnais **40/0**. ✅
...
- **Preuve** : commits `f2929c6` (CHECK), `3eaef2f` (vertical). Harnais **41/0**. ✅
...
- **Preuve** : commits `ba6a117` (CHECK), `9effd4f` (vertical). Domaine 18/18 + harnais **72/0** + bisectabilité (`BISECT_A_BUILD_OK`). ✅
...
- **Preuve** : commit `8c91135` (back) + `2e5789b` (front). Domaine 3/3 + harnais **20/0** + rendu PHP **26/0** (dérivation serveur, tests *killer*). ✅
...
1. ⚠️ **CSRF front (transverse)** : les formulaires POST du front (transitions projet,
...
2. ⚠️ **Badges/Gagner/Score (tables) orphelines** : la gamification réelle est *stateless*
...
4. ⚠️ **Front absent pour certains verticals** : récupération pro (#26/#27) n'a pas d'UI ;
   l'annonce n'a pas de vue « mes annonces » propriétaire. Les routes serveur existent
   (noms de champs DTO préservés) — UI à construire.
...
6. ⚠️ **`forum` front** : doit attacher l'en-tête `Authorization` sur les mutations.

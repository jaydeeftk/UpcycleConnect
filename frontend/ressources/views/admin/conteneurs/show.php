...
<div class="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <p class="text-xs uppercase font-bold text-slate-400 mb-2">Statut</p>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <p class="text-xs uppercase font-bold text-slate-400 mb-2">Capacité max</p>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        <p class="text-xs text-slate-400 mt-2"><?= (int)($conteneur['occupation'] ?? 0) ?> objet(s) en stock · <?= (int)($conteneur['nb_demandes'] ?? 0) ?> dépôt(s) validé(s)</p>
    </div>
</div>

<div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
    <div class="px-6 py-4 border-b border-slate-200">
        <h3 class="font-bold text-slate-800">Dépôts dans cette box</h3>
    </div>
    <div class="overflow-x-auto">
    <table class="w-full text-left">
        <thead>
            <tr class="bg-slate-50 text-slate-500 text-xs uppercase tracking-wider">
                <th class="p-4 font-semibold">Particulier</th>
        ...
        <tbody class="divide-y divide-slate-100">
            <?php if (empty($demandes)): ?>
                <tr><td colspan="6" class="p-8 text-center text-slate-400 italic">Aucun dépôt dans cette box.</td></tr>
                ...
                <td class="p-4 text-right">
                    ...
                    <div class="flex justify-end gap-2">
                        <form method="POST" action="/admin/demandes/valider/<?= $d['id'] ?>">
                            <button type="submit" class="w-8 h-8 flex items-center justify-center bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-600 hover:text-white transition-colors" title="Valider"><i class="fas fa-check text-xs"></i></button>
                        </form>
                        <form method="POST" action="/admin/demandes/refuser/<?= $d['id'] ?>">
                            <button type="submit" class="w-8 h-8 flex items-center justify-center bg-rose-50 text-rose-600 rounded-lg hover:bg-rose-600 hover:text-white transition-colors" title="Refuser"><i class="fas fa-times text-xs"></i></button>
                        </form>
                    </div>
                    <?php else: ?>

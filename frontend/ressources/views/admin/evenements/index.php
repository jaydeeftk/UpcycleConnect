<div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h2 class="text-2xl font-bold text-slate-800"><?= t('adm_events_title', 'Événements') ?></h2>
        <p class="text-slate-500"><?= t('adm_events_subtitle', 'Gestion des événements et ateliers') ?></p>
    </div>
    <div class="flex gap-3">
        <input type="text" id="filter-events" onkeyup="filterTable('filter-events','table-events')"
            placeholder="<?= t('adm_filter', 'Filtrer...') ?>"
        ...
        <a href="/admin/evenements/create" class="bg-emerald-500 text-white px-4 py-2 rounded-lg hover:bg-emerald-600 transition-colors font-medium text-sm whitespace-nowrap">
            <i class="fas fa-plus mr-2"></i><?= t('adm_btn_create', 'Créer') ?>
        </a>
    </div>
</div>
    ...
    <table id="table-events" class="w-full text-left border-collapse">
        <thead>
            <tr class="bg-slate-50 border-b border-slate-200 text-slate-500 text-xs uppercase tracking-wider">
                <th class="p-4 font-semibold"><?= t('adm_events_col_event', 'Événement') ?></th>
                <th class="p-4 font-semibold"><?= t('adm_events_col_date_place', 'Date & Lieu') ?></th>
                <th class="p-4 font-semibold"><?= t('adm_col_price', 'Prix') ?></th>
                <th class="p-4 font-semibold"><?= t('adm_events_col_animateur', 'Animateur') ?></th>
        ...
        <tbody class="divide-y divide-slate-100">
            <?php if (empty($evenements)): ?>
                <tr><td colspan="6" class="p-8 text-center text-slate-400 italic"><?= t('adm_events_empty', 'Aucun événement.') ?></td></tr>
            ...
            <tr class="hover:bg-slate-50 transition-colors">
                ...
                <td class="p-4">
                    ...
                    <?php else: ?>
                    <span class="text-slate-400 text-sm"><?= t('adm_events_free', 'Gratuit') ?></span>
                    <?php endif; ?>
                </td>
                <td class="p-4 text-sm text-slate-600">
...
<div id="modal-evt" class="hidden fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
            <h3 class="text-lg font-bold text-slate-800" id="evt-modal-titre"><?= t('adm_events_modal_title', 'Détail événement') ?></h3>
        ...
        <div class="p-6 space-y-4 text-sm">
            <div class="grid grid-cols-2 gap-4">
                <div><p class="text-xs text-slate-400 uppercase font-bold mb-1"><?= t('adm_events_modal_date', 'Date') ?></p><p id="evt-date" class="font-medium text-slate-700"></p></div>
                <div><p class="text-xs text-slate-400 uppercase font-bold mb-1"><?= t('adm_events_modal_place', 'Lieu') ?></p><p id="evt-lieu" class="font-medium text-slate-700"></p></div>
                <div><p class="text-xs text-slate-400 uppercase font-bold mb-1"><?= t('adm_col_price', 'Prix') ?></p><p id="evt-prix" class="font-bold text-emerald-600"></p></div>
                <div><p class="text-xs text-slate-400 uppercase font-bold mb-1"><?= t('adm_events_modal_capacity', 'Capacité') ?></p><p id="evt-capacite" class="font-medium text-slate-700"></p></div>
                <div><p class="text-xs text-slate-400 uppercase font-bold mb-1"><?= t('adm_col_status', 'Statut') ?></p><p id="evt-statut" class="font-medium text-slate-700"></p></div>
                <div><p class="text-xs text-slate-400 uppercase font-bold mb-1"><?= t('adm_events_col_animateur', 'Animateur') ?></p><p id="evt-animateur" class="font-medium text-slate-700"></p></div>
            </div>
            <div><p class="text-xs text-slate-400 uppercase font-bold mb-1"><?= t('adm_events_label_description', 'Description') ?></p><p id="evt-desc" class="text-slate-600 leading-relaxed"></p></div>
        </div>
    </div>
</div>

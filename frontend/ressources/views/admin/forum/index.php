<div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h2 class="text-2xl font-bold text-slate-800"><?= t('adm_forum_title', 'Modération Forum') ?></h2>
        <p class="text-slate-500"><?= t('adm_forum_subtitle', 'Gestion des sujets et discussions') ?></p>
    </div>
    ...
    <table id="table-forum" class="w-full text-left border-collapse">
        <thead>
            <tr class="bg-slate-50 border-b border-slate-200 text-slate-500 text-xs uppercase tracking-wider">
                <th class="p-4 font-semibold"><?= t('adm_forum_col_sujet', 'Sujet') ?></th>
                <th class="p-4 font-semibold"><?= t('adm_col_author', 'Auteur') ?></th>
                <th class="p-4 font-semibold"><?= t('adm_forum_col_replies', 'Réponses') ?></th>
        ...
        <tbody class="divide-y divide-slate-100">
            <?php if (empty($sujets)) { ?>
                <tr>
                    <td colspan="6" class="p-8 text-center text-slate-400 italic"><?= t('adm_forum_empty', 'Le forum est vide pour le moment.') ?></td>
                </tr>
            <?php } else { ?>
                ...
                <tr class="hover:bg-slate-50 transition-colors group">
                    <td class="p-4">
                        <p class="font-bold text-slate-800"><?= htmlspecialchars($sujet['titre'] ?? t('adm_forum_default_title', 'Sujet sans titre')) ?></p>
                    ...
                    <td class="p-4">
                        <div class="flex items-center">
                            ...
                            <span class="text-sm font-medium text-slate-700"><?= htmlspecialchars($sujet['auteur'] ?? t('adm_forum_default_author', 'Utilisateur')) ?></span>
                        </div>
                    </td>
                    <td class="p-4">
                    ...
                    <td class="p-4 text-right space-x-2">
                        <a href="/conseils/forum/<?= $idSujet ?>" target="_blank"
                           class="w-8 h-8 inline-flex items-center justify-center rounded-lg bg-blue-50 text-blue-500 hover:bg-blue-500 hover:text-white transition-colors" title="<?= t('adm_forum_view_subject', 'Voir le sujet') ?>">

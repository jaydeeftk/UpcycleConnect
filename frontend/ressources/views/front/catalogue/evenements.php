<section class="max-w-7xl mx-auto px-6 lg:px-10 py-16">
        ...
        <form method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div>
                ...
                <select name="tarif" class="select select-bordered w-full select-sm">
                    <option value="">Tous</option>
                    <option value="gratuit" <?= ($_GET['tarif'] ?? '') === 'gratuit' ? 'selected' : '' ?>>Gratuit</option>
                    <option value="payant" <?= ($_GET['tarif'] ?? '') === 'payant' ? 'selected' : '' ?>>Payant</option>
                </select>
            </div>
            <div>
            ...
            <div>
                ...
                <input type="text" name="localisation" placeholder="Ville ou arrondissement" value="<?= htmlspecialchars($_GET['localisation'] ?? '') ?>" class="input input-bordered w-full input-sm">
            </div>
            <div>
    ...
    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        <?php foreach ($evenements as $ev):
            $colorClass = $typeColors[$ev['type']] ?? 'bg-gray-50 text-gray-600';
        ?>
            <div class="bg-base-100 rounded-2xl shadow-sm overflow-hidden hover:shadow-md transition">
                <div class="w-full h-36 bg-blue-50 flex items-center justify-center relative">
                    <i class="fas <?= $ev['icon'] ?> text-5xl text-blue-200"></i>
                    <div class="absolute top-3 left-3">
                        <span class="badge badge-sm <?= $colorClass ?> border-0"><?= $ev['type'] ?></span>
                    </div>
                    <div class="absolute top-3 right-3">
                        <?php if ($ev['prix'] === 0): ?>
                            <span class="badge badge-success badge-sm">Gratuit</span>
                        <?php else: ?>
                            <span class="badge badge-ghost badge-sm"><?= $ev['prix'] ?>€</span>
                        <?php endif; ?>
                    </div>
                </div>
                <div class="p-6">
                    <h3 class="text-lg font-semibold mb-2"><?= htmlspecialchars($ev['titre']) ?></h3>
                    <p class="text-sm text-base-content/60 mb-4 line-clamp-2"><?= htmlspecialchars($ev['description']) ?></p>

                    <div class="space-y-2 mb-4 text-xs text-base-content/50">
                        <div><i class="fas fa-calendar-alt mr-2"></i><?= $ev['date'] ?> · <?= $ev['heure'] ?></div>
                        <div><i class="fas fa-map-marker-alt mr-2"></i><?= $ev['lieu'] ?></div>
                        <div><i class="fas fa-users mr-2"></i><?= $ev['participants'] ?> participant(s)</div>
                    </div>

                    <div class="flex items-center justify-between">
                        <span class="text-xl font-bold"><?= $ev['prix'] === 0 ? 'Gratuit' : $ev['prix'] . '€' ?></span>
                        <a href="/UpcycleConnect-PA2526/frontend/public/evenements/<?= $ev['id'] ?>" class="btn btn-neutral btn-sm">
                            Participer
                        </a>
                    </div>
                </div>

<section class="max-w-2xl mx-auto px-6 lg:px-10 py-16">
    <div class="mb-10">
        <h1 class="text-3xl font-bold mb-2"><?= t('contact_title', 'Nous contacter') ?></h1>
        <p class="text-base-content/60"><?= t('contact_subtitle', 'Envoyez-nous un message, nous vous répondrons rapidement.') ?></p>
    </div>

    <?php if (!empty($_SESSION['contact_success'])): ?>
        <div class="alert alert-success mb-6">
            <i class="fas fa-check-circle"></i>
            <span><?= t('contact_success', 'Votre message a bien été envoyé !') ?></span>
        </div>
    ...
    <div class="bg-base-100 rounded-2xl shadow-sm border border-base-300 p-8">
        ...
        <?= csrf_field() ?>
            ...
            <button type="submit" class="btn btn-neutral w-full">
                <i class="fas fa-paper-plane mr-2"></i><?= t('contact_send_btn', 'Envoyer le message') ?>
            </button>
        </form>
        ...
        <?php if (!isset($_SESSION['user'])): ?>
            <div class="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-xl text-sm text-amber-700">
                ...
                <?= t('contact_login_required_before', 'Vous devez être') ?> <a href="/login" class="font-semibold underline"><?= t('contact_login_required_link', 'connecté') ?></a> <?= t('contact_login_required_after', 'pour envoyer un message.') ?>
            </div>
        <?php endif; ?>
    </div>
</section>

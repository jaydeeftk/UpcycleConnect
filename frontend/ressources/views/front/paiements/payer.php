<section class="max-w-lg mx-auto px-6 py-16">
    ...
    <div class="mb-8">
        <a href="javascript:history.back()" class="flex items-center gap-2 text-sm text-base-content/50 hover:text-base-content transition mb-6">
            <i class="fas fa-arrow-left"></i> <?= t('paypay_back', 'Retour') ?>
        </a>
        <h1 class="text-2xl font-bold"><?= t('paypay_title', 'Finaliser le paiement') ?></h1>
    ...
    <div class="bg-base-100 rounded-2xl shadow-sm border border-base-300 p-6 mb-6">
        ...
        <div class="flex justify-between items-center pt-3">
            <span class="font-semibold"><?= t('paypay_total', 'Total') ?></span>
    ...
    <div class="bg-base-100 rounded-2xl shadow-sm border border-base-300 p-6 mb-6">
        ...
        <button id="btn-payer" onclick="handlePaiement()"
            ...
            <i class="fas fa-lock mr-2"></i> <?= t('paypay_pay_btn', 'Payer') ?> <?= htmlspecialchars($_GET['montant'] ?? '0') ?>€
        </button>

        <p class="text-xs text-base-content/40 text-center mt-3">
            <i class="fas fa-shield-alt mr-1"></i>
            <?= t('paypay_secure_notice', 'Paiement 100% sécurisé — vos données bancaires ne sont jamais stockées') ?>
        </p>
    </div>
...
async function handlePaiement() {
    ...
    btn.disabled = true;
    btn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i> <?= t('paypay_processing', 'Traitement...') ?>';
    ...
    try {
        ...
        if (json.success && json.data && json.data.checkout_url) {
            ...
            toast('<?= t('paypay_checkout_error', 'Erreur lors de la création de la session de paiement') ?>');
            btn.disabled = false;
            btn.innerHTML = '<i class="fas fa-lock mr-2"></i> <?= t('paypay_pay_btn', 'Payer') ?> <?= htmlspecialchars($_GET['montant'] ?? '0') ?>€';
        }
    } catch(e) {
        console.error(e);
        btn.disabled = false;

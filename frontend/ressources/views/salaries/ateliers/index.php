        ...
        <tbody class="divide-y divide-gray-200">
            ...
            <tr>
                <td class="px-6 py-4 text-sm font-medium text-gray-800">
                    <?= htmlspecialchars($atelier['theme'] ?? '') ?>
                </td>
                <td class="px-6 py-4 text-sm text-gray-600">
                    <?= $atelier['date_atelier'] ? date('d/m/Y H:i', strtotime($atelier['date_atelier'])) : '—' ?>
                </td>
                <td class="px-6 py-4 text-sm text-gray-600">
                    <?= htmlspecialchars($atelier['lieu'] ?? '—') ?>
                </td>
                <td class="px-6 py-4 text-sm text-gray-600">
                    <?= htmlspecialchars($atelier['createur'] ?? '—') ?>
                </td>
                <td class="px-6 py-4">
                    <?php
                ...
                <td class="px-6 py-4 text-sm text-gray-600">
                    <?= htmlspecialchars($atelier['auteur'] ?? 'Inconnu') ?>
                </td>
                <td class="px-6 py-4">
                    <div class="flex gap-2">
                        ...
                        <form method="POST" action="/salaries/ateliers/<?= $atelier['id_atelier'] ?>/delete" class="inline"
                           onsubmit="return ucConfirm(this, '<?= t('sal_ateliers_delete_confirm', 'Supprimer cet atelier ?') ?>')">
                            <?= csrf_field() ?>
...
<script>
function openEditModal(id, theme, date, lieu) {
